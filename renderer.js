...
export default class Renderer {
	...
	renderTiles(map) {
		for (var x = 0; x < map.width; x++)
			for (var y = 0; y < map.height; y++)
				this.renderTile(new TileResult(x, y, map.tiles[x][y]), map);
	}
	renderTile(tileresult, optionalMap) {
		throw new Error("Unimplemented renderTile!");
	}
	overlayTiles(tileresults, spriteid, map) {
		for (var tileresult of tileresults)
			this.overlayTile(tileresult, spriteid, map);
	}
	...
	renderBuildings(map) {
		for (var x = 0; x < map.width; x++)
			for (var y = 0; y < map.height; y++)
				if (map.tiles[x][y].hasBuilding)
					this.renderBuilding(new TileResult(x, y, map.tiles[x][y]), map);
	}
	...
	renderUnits(map) {
		for (var player of map.players)
			for (var unit of player.units)
				this.renderUnit(unit, player.style, map);
	}
	...
	redrawTile(tileresult, map) {
		...
		if (unitPlayer = map.players.find((player) =>
			(tileUnit = player.units.find((unit) =>
